...
export default function AdminPanel({ 
  ...
  const handleResetToDemo = () => {
    if (confirm('Konfirmasi: Reset database ke data demo bawaan showroom (7 unit motor lengkap)?')) {
      setUnits(initialUnits);
      if (setSalesList) setSalesList(initialSalesList);
      localStorage.removeItem('maharga_units_v2');
      localStorage.removeItem('maharga_sales_v2');
      alert('Data showroom telah di-reset ke data bawaan preview.');
    }
  };

  const handleClearAllUnits = () => {
    if (confirm('PERINGATAN: Apakah Anda yakin ingin MENGOSONGKAN semua unit stok untuk memulai input baru showroom asli?')) {
      setUnits([]);
      localStorage.removeItem('maharga_units_v2');
      alert('Semua data unit stok telah dikosongkan. Anda dapat mulai menginput unit motor fisik Anda.');
    }
  };

  // ==========================================
  // 2. FILE MANAGER & DB STATE
  // ==========================================
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] text-zinc-500 ml-2 font-mono hidden sm:inline">mahargamotor.com</span>
            </div>
          </div>

          <div className="flex items-center gap-3 text-xs">
            <span className="text-zinc-400 hidden sm:inline font-mono">SSL Aktif</span>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pt-4 sm:pt-5">
                ...
                {selectedUnit ? (
                  <div className="bg-zinc-900/90 rounded-xl border border-zinc-800 p-4 sm:p-5 space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      {/* Preview Box if user picked a local file */}
                      {uploadPreview && (
                      ...
                      {/* Option C: Preset Quick Photos */}
                      <div className="pt-2 border-t border-zinc-800/80 space-y-2">
                        <span className="text-[11px] text-zinc-400 font-medium block">
                          Pilihan Cepat Foto HD (Koleksi Showroom):
                        </span>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                          {presetPhotos.map((preset, idx) => (
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-zinc-950 border border-zinc-800 hover:border-amber-500/60 text-left flex items-center gap-2 group transition-colors"
                            >
                              <img src={preset.url} alt={preset.title} className="w-8 h-8 rounded object-cover shrink-0" />
                              <span className="text-[10px] text-zinc-300 group-hover:text-amber-300 truncate">
                                {preset.title}
                              </span>
                            </button>
                          ))}
                    ...
                    {/* Active Photos Gallery for this Unit */}
                    <div className="pt-4 border-t border-zinc-800 space-y-3">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {(selectedUnit.images || [selectedUnit.images?.[0] || 'https://images.unsplash.com/photo-1558981403-c5f9899a28bc?auto=format&fit=crop&w=800&q=80']).map((imgUrl, index) => (
                          <div 
                            key={index}
                  ...
                  <div className="p-10 text-center text-zinc-500 text-xs bg-zinc-900/90 rounded-xl border border-zinc-800">
                    Pilih unit motor di sebelah kiri untuk mengelola fotonya.
                  </div>
                )}
              </div>
            </div>
        ...
        {/* ========================================================= */}
        {/* MODULE 2: FILE MANAGER (PUBLIC_HTML) */}
        {/* ========================================================= */}
        {activeModule === 'files' && (
          <div className="space-y-4">
        ...
        {/* ========================================================= */}
        {/* MODULE 5: BACKUP, EXPORT & DATABASE RESET WIZARD */}
        {/* ========================================================= */}
        {activeModule === 'backup' && (
          <div className="bg-zinc-900/90 rounded-xl border border-zinc-800 p-5 space-y-5 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 space-y-2.5">
                ...
                <p className="text-zinc-400 text-[11px]">
                  Kembalikan 7 unit motor bawaan (Vario, NMAX, Scoopy, PCX, Aerox, KLX, Beat).
                </p>
                <button
              ...
              {/* Card 4: Clear All Units (Fresh Start) */}
